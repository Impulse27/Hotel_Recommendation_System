...
datad = pickle.load(open('countries_dict.pkl','rb'))
...
selected_tag= st.selectbox('Enter the purpose of trip',
tags)

if st.button('Recommend'):
    m = recommend_hotel(str(selected_country), str(tags))
    m = pd.DataFrame(m)
    m = m.iloc[:6, ]
    m = m[['Hotel_Name', 'Average_Score', 'Hotel_Address']]
    st.table(m)
